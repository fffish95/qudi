...
class LocalNICard(Base, SlowCounterInterface, ConfocalScannerInterface):
    ...
    nicard:
        module.Class: 'local_ni_card.LocalNICard'
        max_counts: 3e7
        ...
        scanner_position_ranges:
            - [0e4, 3e4]
        default_scanner_clock_frequency: 2500 # optional, in Hz
        scanner_clock_channel: '/Dev3/Ctr0'
    ...
    # config options
    _photon_sources = ConfigOption('photon_sources', list(), missing='error')
    ...
    # local laser scanning
    _scanner_ao_channels = ConfigOption('scanner_ao_channels', missing='info')
    _scanner_counter_channels = ConfigOption('scanner_counter_channels', list(), missing='info')
    _scanner_voltage_ranges = ConfigOption('scanner_voltage_ranges', missing='info')
    _scanner_position_ranges = ConfigOption('scanner_position_ranges', missing='info')
    _default_scanner_clock_frequency = ConfigOption('default_scanner_clock_frequency', 2500, missing='info')
    _scanner_clock_channel = ConfigOption('scanner_clock_channel', missing='info')

    _ramp_ao_channels = ConfigOption('ramp_ao_channels', missing='info')
    _ramp_voltage_ranges = ConfigOption('ramp_voltage_ranges', missing='info')

    # local slow counter
    _counter_channels = ConfigOption('counter_channels', missing='info')
    _clock_channel = ConfigOption('clock_channel', missing='info')
    ...
    def set_up_clock(self, clock_frequency=None, clock_channel=None, scanner=False, idle=False):
        ...
        @param bool scanner: if set to True method will set up a clock function
                             ...
                             counter will be set.
        @param bool idle: set the resting state of the output terminal
                                True  = 'Voltage High/Rising Edge'
                                False = 'Voltage Low/Falling Edge'

        @return int: error code (0:OK, -1:error)
        
        Create a clock task with specific clock frequency,
         including create digital clock channel and assign buffer to it.
        scanner = True:
        set _scanner_clock_frequency and _scanner_clock_channel,
         task_name: 'ScannerClock' *TaskHandle: _scanner_clock_daq_task
        scanner = False: task started
        set _clock_frequency and _clock_channel,
         task_name: 'CounterClock' *TaskHandle: _clock_daq_task
    ...
    def set_up_counter(self,
        ...
        Prerequisites: set_up_clock
        If clock_channel is not None:
         The scan program does not use the clock generated by the setup function,
         but another specified internal clock, the clock is not recorded by global variable 

        Create and start counter tasks for counter which samples the photon ticks within semi periode of the clock pulse
        task_name: 'Counter0' *TaskHandle: _counter_daq_tasks
        1 counter task should correspond to 1 photon source
        buffer size 1000
        """
    ...
    def get_counter(self, samples=None):
        ...
        Prerequisites: set_up_counter
        Get counts from the counter task and integrate the counts from semi period to the total period
        """
        ...
        # add up adjoint pixels to integrate the counts from semi period to the total period
        real_data = count_data[:, ::2]
        real_data += count_data[:, 1::2]

        # initial counts for counter channels to 222
    ...
    def close_counter(self, scanner=False):
        ...
        Stop counter tasks and delete the tasks
        If scanner = True:
         Stop counter tasks for scannning and delete the tasks
        """
        error = 0
        if scanner:
    ...
    def close_clock(self, scanner=False):
        ...
        Stop clock tasks and delete the tasks
        If scanner = True:
         Stop clock tasks for scannning and delete the tasks
        """
        if scanner:
    ...
    def _start_analog_output(self):
        """ Create the task for analog output.
             *TaskHandle: _scanner_ao_task
    ...
    def set_up_scanner(self,
        ...
        Prerequisites: set_up_scanner_clock
        If clock_channel is not None:
         The scan program does not use the clock generated by the setup function,
         but another specified internal clock, the clock is recorded by self._my_scanner_clock_channel
        
        If scanner_ao_channels is not None:
         update to self._scanner_ao_channels

        Create counter tasks for scanning which samples the photon ticks within semi periode of the clock pulse
        task_name: 'ScannerCounter0' *TaskHandle: _scanner_counter_daq_tasks
        1 counter task should correspond to 1 photon source
        ...
        try:
            ...
            for i, ch in enumerate(my_counter_channels):
                ...
                daq.DAQmxCreateCISemiPeriodChan(
                    ...
                    0,
                    # Expected maximum count value
                    self._max_counts / 2 / self._scanner_clock_frequency,
                ...
                # Specify the terminal of the timebase which is used for the counter:
                # Define the source of ticks f-or the counter as self._photon_source for
                # the Scanner Task.
                daq.DAQmxSetCICtrTimebaseSrc(
                    # define to which task to# connect this function
                    task,
    ...
    def scanner_set_position(self, x=None, y=None, z=None, a=None):
        ...
        Directly write the position to the hardware
         only works in the cursor loop
        """
    ...
    def _write_scanner_ao(self, voltages, length=1, start=False):
        ...
        @param float[n][length] voltages: array of n-part tuples defining the voltage
                                    points
    ...
    def scan_line(self, line_path=None, pixel_clock=False):
        ...
        Scan the line_path, the input should be the position array
        Prerequisites: set_up_scanner
        Get counts from the counter task and integrate the counts from semi period to the total period
        """
        ...
        try:
           ...
           # daq.DAQmxWaitUntilTaskDone(
            ...
            # the clock:
            real_data = scan_data[:, ::2]
            real_data += scan_data[:, 1::2]

            # initial counts for counter channels to 2
            all_data = np.full(
                (len(self.get_scanner_count_channels()), self._line_length), 2, dtype=np.float64)
            all_data[0:len(real_data)] = np.array(
                real_data * self._scanner_clock_frequency, np.float64)
    ...
    def _set_up_line(self, length=100):
        ...
        Connect the scanner task with the scanner clock
        Switch the scanner clock task from generating continous samples mode to only generate a limited number of samples
        Assign buffer to counter task for semi period measurement
        """
        if self._scanner_counter_channels and len(self._scanner_counter_daq_tasks) < 1:
            self.log.error('Configured counter is not running, cannot scan a line.')
            return np.array([[-1.]])
        ...
        try:
            ...
            if length < np.inf:
                ...
                # Connect the scanner task with the scanner clock and assign buffer to it
            ...
            for i, task in enumerate(self._scanner_counter_daq_tasks):
                ...
                daq.DAQmxCfgImplicitTiming(
                    ...
                    # only a limited number of counts
                    daq.DAQmx_Val_FiniteSamps,
                    # count twice for each voltage +1 for safety( since we do not read the first sample)
    ...
    def close_scanner(self):
        ...
        b = self.close_counter(scanner=True)
    ...
    def _start_ramp_ao(self):
        """ Create the task for ramp analog output.
             *TaskHandle: _ramp_ao_task

        @return int: error code (0:OK, -1:error)
        """
        try:
            # If an analog task is already running, kill that one first
            if self._ramp_ao_task is not None:
                # stop the analog output task
                daq.DAQmxStopTask(self._ramp_ao_task)

                # delete the configuration of the analog output
                daq.DAQmxClearTask(self._ramp_ao_task)

                # set the task handle to None as a safety
                self._ramp_ao_task = None

            # initialize ao channels 
            # Define at first the type of the variable as a Task:
            self._ramp_ao_task = daq.TaskHandle()

            # create the actual analog output task on the hardware device. Via
            # byref you pass the pointer of the object to the TaskCreation function:
            daq.DAQmxCreateTask('RampAO', daq.byref(self._ramp_ao_task))
            for n, chan in enumerate(self._ramp_ao_channels):
                # Assign and configure the created task to an analog output voltage channel.
                daq.DAQmxCreateAOVoltageChan(
                    # The AO voltage operation function is assigned to this task.
                    self._ramp_ao_task,
                    ...
                    chan,
                    # assign a name for that channel
                    'Ramp AO Channel {0}'.format(n),
                    # minimum possible voltage
                    self._ramp_voltage_ranges[n][0],
                    # maximum possible voltage
                    self._ramp_voltage_ranges[n][1],
                    # units is Volt
                    daq.DAQmx_Val_Volts,
                    # empty for future use
                    '')
        except:
            self.log.exception('Error starting ramp analog output task.')
            return -1
        return 0



    def _stop_ramp_ao(self):
        """ Stops the analog output for ramp and clear the task

        @return int: error code (0:OK, -1:error)
        """
        if self._ramp_ao_task is None:
            return -1
        retval = 0
        try:
            # stop the analog output task
            ...
            self.log.exception('Error stopping ramp analog output.')
            retval = -1
        # clear the task
        try:
            daq.DAQmxClearTask(self._ramp_ao_task)
            ...
            self.log.exception('Could not clear ramp AO Out Task.')
            retval = -1
        return retval



    def _write_ramp_ao(self, voltages, length=1):
        """Writes a set of voltages to the ramp analog outputs, the _ramp_ao_task starts immediately 
        ...
        # The error code of this variable can be asked with .value to check
        # whether all channels have been written successfully.
        self._RAMPAONwritten = daq.int32()
        # write the voltage instructions for the analog output to the hardware
        daq.DAQmxWriteAnalogF64(
            # write to this task
            self._ramp_ao_task,
            ...
            voltages,
            # The actual number of samples per channel successfully written to the buffer
            daq.byref(self._RAMPAONwritten),
            # Reserved for future use. Pass NULL(here None) to this parameter
            None)
        return self._RAMPAONwritten.value
    ...
    def write_single_trigger(self, channel_name, length = 10):
        """
        Function for performing a single pulse for the trigger (line trigger)
        @param str channel_name: Name of the channel which should be controlled
                                    for example ('/Dev1/PFI9')
        @length: sample number to right, default is 10
        """
        if channel_name is None:
            self.log.error('No channel for line trigger output specified')
            return -1
        else:

            self.single_trigger_out_task = daq.TaskHandle()
            self.single_trigger_data = np.array(daq.c_uint32(0xffffffff))
            for i in range(length-1):
                np.append(self.single_trigger_data, daq.c_uint32(0xffffffff))
            np.append(self.single_trigger_data, daq.c_uint32(0x0))
            self.single_trigger_read = daq.c_int32()
            self.single_trigger_samples_channel = daq.c_int32(length+1)
            daq.DAQmxCreateTask('SingleTriggerOut', daq.byref(self.single_trigger_out_task))
            daq.DAQmxCreateDOChan(self.single_trigger_out_task, channel_name, "", daq.DAQmx_Val_ChanForAllLines)
            daq.DAQmxStartTask(self.single_trigger_out_task)
            daq.DAQmxWriteDigitalU32(self.single_trigger_out_task, self.single_trigger_samples_channel, True,
                                        self._RWTimeout, daq.DAQmx_Val_GroupByChannel,
                                        self.single_trigger_data, self.single_trigger_read, None)

            daq.DAQmxStopTask(self.single_trigger_out_task)
            daq.DAQmxClearTask(self.single_trigger_out_task)
            return 0
